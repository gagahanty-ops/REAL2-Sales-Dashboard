...
## План 3 выполнен целиком (2026-09-19)

За одну сессию после восстановления окружения закрыты задачи 3–7:

| Задача | Что сделано |
| --- | --- |
| 3 | Таймлайны этапов, вехи, `normalizeSyncRun` |
| 4 | Единая политика кодов качества, гейты публикации, приёмка админом |
| 5 | Канонические метрики и golden-набор |
| 6 | Неизменяемые снимки метрик и версионированные планы |
| 7 | Сквозной контракт сырьё→снимок, идемпотентность, runbook сверки |

Итоговый гейт: unit 409/409, contracts 19/19 (дважды), integration 127/127, security 28/28, lint/typecheck/build/db lint чисто.

Два дефекта нашли сами контракты, а не ревью: сборщик снимка читал системные часы (контрольная сумма менялась между пересборками) и golden-набор содержал копеечную цену, которую amoCRM физически не присылает.

Ни нормализация, ни сборка снимков пока не подключены к расписанию воркера — это отдельное изменение.
