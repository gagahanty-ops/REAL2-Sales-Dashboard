...
## Задача 3 выполнена (2026-09-19)

Выведение таймлайнов и вех: `packages/domain/src/leads/build-history.ts`, `packages/db/src/normalize-run.ts`, `apps/worker/src/jobs/normalize-sync-run.ts`. 33 доменных теста и 10 интеграционных, 14 внесённых дефектов — все пойманы.

Главный урок задачи: первое решение (удалять нормализованные строки сделки, ушедшей из воронки) противоречило двум зафиксированным правилам — METRICS_CATALOG §11 («ни одна проблемная сделка не удаляется») и security-тесту задачи 1 про append-only историю. Миграция с правами на delete отозвана, база пересобрана из миграций, решение заменено на эскалацию проблемы до `blocking`.
