...
/**
 * The local database is shared with every other suite. Clearing only the two
 * tables this file writes to made it depend on the order the suites ran in: a
 * pipeline configuration left by another run holds a foreign key on the
 * connection this test deletes.
 */
async function clearDependentFixtures(): Promise<void> {
...
afterEach(clearDependentFixtures);
